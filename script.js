...
if (getCookie("userLogin")) {
    ...
    form.addEventListener("submit", () => {
        ...
        if (pwNice && loginNice) {
            ...
            let response = fetch(`http://localhost:81/lesson84%20(trying%20to%20send%20cookie)/index.php?login=${form.elements.login.value}&pw=${form.elements.password.value}`)
            response.then(response => {
                return response.text();
            }).then(data => {
                alert(data);
